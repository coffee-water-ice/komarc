...
def normalize_publisher_name_stage1(name):
    return re.sub(r"\s|\(.*?\)|주식회사|㈜|도서출판|출판사|프레스", "", name).lower()

def normalize_publisher_name_stage2(name):
...
def get_publisher_location(publisher_name, publisher_data):
    try:
        target = normalize_publisher_name_stage1(publisher_name)
        for row in publisher_data:
            if len(row) < 3: continue
            sheet_name, region = row[1], row[2]
            if normalize_publisher_name_stage1(sheet_name) == target:
...
def get_mcst_address_with_normalization(publisher_name):
    ...
    name_stage2 = normalize_publisher_name_stage2(publisher_name)
...
# --- Streamlit UI ---
# =========================
st.title("📚 ISBN → KORMARC 변환기 (KPIPA + 문체부 보완 + 단계별 정규화)")
            ...
            if location_raw == "출판지 미상":
                ...
                if crawl_err: debug_messages.append(f"❌ KPIPA 크롤링 실패: {crawl_err}")
                    ...
                    if new_location != "출판지 미상":
                        ...
                        location_display = normalize_publisher_location_for_display(new_location)
                        debug_messages.append(f"🏙️ KPIPA 기반 재검색 결과: {new_location}")

            # 5) 출판지 여전히 미상 → 문체부 검색
            mcst_address, mcst_results = ("",[])
            if location_raw == "출판지 미상":
                mcst_address, mcst_results = get_mcst_address_with_normalization(publisher)
                if mcst_results:
                    location_display = normalize_publisher_location_for_display(mcst_results[0][2])
